"""STA issue detectors driven by YAML ``issues`` / ``detect`` blocks."""
...
def detect_missing_followup(
    ...
    group_by = list(detect.get("group_by") or ["sta_mac", "ifname"])
    within_sec = float(detect.get("within_sec", 5))
    if not trigger or not expect:
        return []

    tr = df.filter(pl.col("event_code") == trigger)
    tr = tr.filter(pl.col("sta_mac").is_not_null() & (pl.col("sta_mac") != ""))
    ex = df.filter(pl.col("event_code") == expect)
    ex = ex.filter(pl.col("sta_mac").is_not_null() & (pl.col("sta_mac") != ""))
    if tr.height == 0:
        return []

    for col in group_by:
        if col not in tr.columns:
            tr = tr.with_columns(pl.lit("").alias(col))
        if col not in ex.columns:
            ex = ex.with_columns(pl.lit("").alias(col))

    tr = tr.sort(group_by + ["timestamp"])
    ex = ex.sort(group_by + ["timestamp"])
    ex = ex.rename({"timestamp": "expect_ts"})

    tol = timedelta(seconds=within_sec)
    try:
        # Inputs are sorted by ``group_by`` + time; Polars cannot verify per-group order when ``by`` is set.
        joined = tr.join_asof(
            ex,
            left_on="timestamp",
            right_on="expect_ts",
    ...
    for row in miss.to_dicts():
        ...
        out.append(
            _issue_base(
                ...
                str(row.get("sta_mac") or ""),
                str(row.get("ifname") or ""),
                str(row.get("wcid") or ""),
                wstart,
                None,
...
def detect_burst_count(
    ...
    sub = sub.filter(pl.col("sta_mac").is_not_null() & (pl.col("sta_mac") != ""))
    if sub.height == 0:
        return []

    for col in group_by:
        if col not in sub.columns:
            sub = sub.with_columns(pl.lit("").alias(col))

    out: List[Dict[str, Any]] = []
    parts = sub.partition_by(group_by) if group_by else [sub]
    for part in parts:
        ...
        for i, j, t_start, t_end in wins:
            ...
            out.append(
                _issue_base(
                    ...
                    meta.get("severity", "medium"),
                    str(meta.get("rca_hint", "")),
                    str(row0.get("sta_mac") or ""),
                    str(row0.get("ifname") or ""),
                    str(row0.get("wcid") or ""),
                    t_start,
                    t_end,
                    evidence,
                )
...
def detect_ordered_sequence(
    ...
    sub = df.filter(pl.col("sta_mac").is_not_null() & (pl.col("sta_mac") != ""))
    if sub.height == 0:
        return []
    for col in group_by:
        if col not in sub.columns:
            ...
            if ok and len(chain_idx) == len(sequence):
                ...
                for idx in chain_idx:
                    evidence.append(
                        {
                            "event": evs[idx],
                            "timestamp": ts[idx].isoformat() if hasattr(ts[idx], "isoformat") else str(ts[idx]),
                        }
                    )
