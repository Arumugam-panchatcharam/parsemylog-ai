...
def polars_etl_per_cpe(user_id: str, project_id: str, serial: str, 
    ...
    try:
        ...
        preserve_sta_issues = False
        if sta_issues.height == 0 and si_path.is_file():
            try:
                existing_si = pl.read_parquet(si_path)
                if (
                    "device_serial" in existing_si.columns
                    and existing_si.filter(pl.col("device_serial") == cpe_serial).height > 0
                ):
                    preserve_sta_issues = True
                    wifi_issue_stats = {
                        **wifi_issue_stats,
                        "sta_issues_preserved": True,
                        "sta_issues_preserve_reason": "empty_run_kept_previous",
                    }
                    logger.info(
                        "Preserving existing sta_issues for %s (empty WiFi run)",
                        cpe_serial,
                    )
            except Exception as e:
                logger.warning("Could not check prior sta_issues for %s: %s", cpe_serial, e)
        if not preserve_sta_issues:
            upsert_replace_device_serial(si_path, sta_issues, cpe_serial)
        results["sta_issues"] = {
            "path": str(si_path),
            "rows": sta_issues.height,
            "wifi_stats": wifi_issue_stats,
            "preserved": preserve_sta_issues,
        }
