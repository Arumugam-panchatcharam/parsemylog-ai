...
def update_templates_table(selected_file, project_data):
    ...
    try:    
        ...
        info = dbm.get_project_file_info(project_id, selected_file)
        if not info:
            return []

        filename, file_path, original_name, file_size, _ = info
...
def get_pipeline_status(project_dir):
    ...
    return queued_files, done_files, all_done
...
def sync_pipeline_status(sync_pipeline_btn, project_data):
    ...
    if ctx.triggered:
        ...
        if prop_id == "sync-pipeline":
            queued_files, done_files, _ = get_pipeline_status(project_dir)
            return queued_files, done_files
...
def update_status(_interval, project_data):
    ...
    queued_files, done_files, all_done = get_pipeline_status(project_dir)
    return queued_files, done_files, all_done
